...
db_path = resource_path("trening_results.db")
root = Tk()
...
def create_table():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS trening_results("
...
def writefun():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS trening_results("
                   ...
                   "number_of_series integer,"
                   "weight float)")
    cursor.execute("INSERT INTO trening_results("
...
def edit_fun():
    ...
    def confirm(new_exercise,new_number_of_repetitions,new_number_of_series,new_weight,id_value):
        connection = sqlite3.connect(db_path)
        cursor = connection.cursor()
        cursor.execute("UPDATE trening_results SET exercise = ?,"
